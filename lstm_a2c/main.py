...
parser.add_argument('--env_name', type=str, default="LunarLander-v2", help='')
...
def main():
    ...
    print('obs size:', img_shape)
    print('action size:', num_actions)

    net = FuN(num_actions, args.horizon)
    optimizer = optim.RMSprop(net.parameters(), lr=0.0025, eps=0.01)
    ...
    histories = torch.zeros([args.num_envs, 8]).to(device)
    ...
    while True:
        ...
        # print(f'episode is {count} and reward is {score_list[-1]}')
        if len(score_list) == 200:
            x_axis = np.arange(len(score_list))
            plt.plot(x_axis,score_list)
            plt.title('reward function')
            plt.xlabel('epoch')
            plt.ylabel('reward')
            plt.savefig('epoch 200 reward1')
            plt.show()
        ...
        # print(f'episode is {count} and loss is {loss_list[-1]}')
        if len(loss_list) ==200:
            x_axis = np.arange(len(loss_list))
            plt.plot(x_axis,loss_list)
            plt.title('loss function')
            plt.xlabel('epoch')
            plt.ylabel('loss')
            plt.savefig('epoch 200 loss1')
            plt.show()
